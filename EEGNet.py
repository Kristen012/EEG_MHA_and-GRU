...
def EEGNet(nb_classes, Chans = 64, Samples = 5*64, 
             dropoutRate = 0.5, kernLength = 64, F1 = 8, 
             D = 2, F2 = 16, norm_rate = 0.25, dropoutType = 'Dropout', 
             env_input_dimension = 10, dilation_filters = 16, time_window=None, layers=3,
    ...
    eeg = Input(shape = (Chans, Samples, 1))
    ...
    for layer_index in range(layers):
        ...
        env_proj_layer = tf.keras.layers.Conv1D(
            dilation_filters,
            kernel_size=3,
            dilation_rate=3**layer_index,
            strides=1,
            activation="relu"[layer_index],
        )
        env_proj_list = [env_proj_layer(env_proj_list) for env_proj_list in env_proj_list]


    # Comparison
    cos = [tf.keras.layers.Dot(1, normalize=True)([flatten, env_proj_list]) for env_proj_list in env_proj_list]
