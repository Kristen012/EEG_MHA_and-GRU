...
def eeg_mha_dc_speech_gru_dc_model(
    ...
    stimuli_proj = [gru_model(x) for x in stimuli_input]
    ...
    # Classification
    out = tf.keras.layers.Dense(5, activation="sigmoid")(
        tf.keras.layers.Flatten()(tf.keras.layers.Concatenate()(cos))
    )
...
if __name__ == "__main__":
    model = dilation_model(time_window=320, eeg_input_dimension=64, env_input_dimension=1, num_mismatched_segments = 4)
    model = eeg_mha_dc_speech_gru_dc_model(time_window=320, eeg_input_dimension=64, env_input_dimension=1, num_mismatched_segments = 4)